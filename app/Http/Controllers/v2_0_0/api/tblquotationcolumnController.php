...
class tblquotationcolumnController extends commonController
{
    ...
    public function formula(Request $request)
    {
        ...
        $quotationcolumn = $this->tbl_quotation_columnModel::all()->whereIn('column_type', ['decimal', 'percentage', 'number'])->where('is_deleted', 0);

        if ($quotationcolumn->count() > 0) {
    ...
    public function column_details(string $id)
    {
        ...
        $columndetails = DB::connection('dynamic_connection')->table('tbl_quotation_columns')->where('is_deleted', 0)->get();
        ;
