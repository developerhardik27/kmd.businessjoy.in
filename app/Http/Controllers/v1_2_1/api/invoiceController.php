...
class invoiceController extends commonController
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($validator->fails()) {
            ...
            if (isset($data['inv_number'])) { //user entered manully inv number 
                // check if inv number already exist.
                $checkinvnumberrec = DB::connection('dynamic_connection')->table('invoices')->where('inv_no', $data['inv_number'])->whereNot('id', $id)->where('is_deleted', 0)->first();
                if ($checkinvnumberrec) {
                    return $this->errorresponse(422, ["inv_number" => ['This number already exists!']]);
                }
    ...
    {
        $existsinvoice = DB::connection('dynamic_connection')->table('invoices')->where('inv_no', $request->inv_number)->where('is_deleted', 0);
