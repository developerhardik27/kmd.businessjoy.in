...
class tblleadController extends commonController
{
    ...
    public $userId, $companyId, $masterdbname, $rp, $tblleadModel, $lead_recent_activityModel, $tblleadhistoryModel, $import_historyModel, $export_historyModel, $lead_settingModel;
    ...
    public function index(Request $request)
    {
        ...
        $leadquery = $this->tblleadModel::leftjoin($this->masterdbname.'.country', 'tbllead.country_id', '=', $this->masterdbname.'.country.id')
            ...
            ->select(
                ...
                'tbllead.budget',
                'tbllead.company',
                'tbllead.audience_type',
                'tbllead.customer_type',
                'tbllead.status',
                'tbllead.last_follow_up',
        ...
        if (isset($filter_assigned_to)) {
            $leadquery->where(function ($query) use ($filter_assigned_to) {
                foreach ($filter_assigned_to as $value) {
                    $query->orWhere('tbllead.assigned_to', 'LIKE', '%' . $value . '%');
                }
            });
        }
        ...
        $filters = [
            'filter_followup_count' => 'tbllead.number_of_follow_up',
            'filter_last_followup_from_date' => 'tbllead.last_follow_up',
            'filter_last_followup_to_date' => 'tbllead.last_follow_up',
            'filter_next_followup_from_date' => 'tbllead.next_follow_up',
            'filter_next_followup_to_date' => 'tbllead.next_follow_up',
            'filter_from_date' => 'tbllead.created_at',
            'filter_to_date' => 'tbllead.created_at',
        ];
        ...
        $mulitplefilters = [
            'filter_lead_status' => 'tbllead.status',
            'filter_lead_stage_status' => 'tbllead.lead_stage',
            'filter_source' => 'tbllead.source'
        ];
    ...
    public function getLeadSettings()
    {

        if ($this->rp['leadmodule']['leadsettings']['view'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }

        $settings = $this->lead_settingModel::first();

        if (!$settings) {
            return response()->json(['status' => 404, 'message' => 'Settings not found'], 404);
        }

        return $this->successresponse(200, 'leadsettings', $settings);
    }

    public function updateLeadSettings(Request $request)
    {
        if ($this->rp['leadmodule']['leadsettings']['edit'] != 1) {
            return $this->successresponse(500, 'message', 'You are Unauthorized');
        }

        $validator = Validator::make($request->all(), [
            'country_field' => 'required|numeric',
            'state_field' => 'required|numeric',
            'city_field' => 'required|numeric',
            'autofill_value' => 'required|string',
            'custom_country' => 'nullable|numeric',
            'custom_state' => 'nullable|numeric',
            'custom_city' => 'nullable|numeric',
        ]);
        ...
        $updateLeadSettings = $this->lead_settingModel::where('id', 1)->update([
            'country' => $request->country_field,
            'state' => $request->state_field,
            'city' => $request->city_field,
            'autofill_value' => $request->autofill_value,
            'country_default_value' => $request->custom_country,
            'state_default_value' => $request->custom_state,
            'city_default_value' => $request->custom_city,
            'updated_by' => $this->userId
        ]);

        if (!$updateLeadSettings) {
            return $this->successresponse(500, 'message', 'Lead settings not succesfully Updated');
        }

        return $this->successresponse(200, 'message', 'Lead settings succesfully Updated');
    }
}
