...
class tblleadhistoryController extends commonController
{
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!is_array($request->leadid) || empty($request->leadid)) {
                return $this->errorresponse(422, ['leadid' => ['No leads selected.']]);
            }

            foreach ($request->leadid as $key => $leadid) {
                ...
                if ($leadhistory) {
                    ...
                    $followup = $request->followup ?? 0;

                    $lead = $this->tblleadModel::find($leadid);
                    ...
                    if ($request->hasFile('attachment')) {
                        ...
                        foreach ($request->file('attachment') as $attachment) {
                            $attachmentname = $lead->first_name . $timestamp . '-' . Str::uuid() . '.' . $attachment->getClientOriginalExtension();

                            $dirPath = public_path('uploads' . DIRECTORY_SEPARATOR . $this->companyId . '/lead/callhistory/' . $timestamp);
                    ...
                    $this->managerecentactivity($leadid, 'call_added', 'Call history added');
                }
            }
    ...
    public function update(Request $request)
    {
        ...
        $leadhistory = $this->tblleadhistoryModel::find($request->history_id);

        if (!$leadhistory) {
            return $this->errorresponse(404, 'Leadhistory record not found.');
        }
        ...
        // ✅ Handle existing attachments (remove if requested)
        $existingAttachments = json_decode($leadhistory->attachment, true) ?? [];

        $removed = json_decode($request->removed_attachments, true);
        if (is_array($removed)) {
            foreach ($removed as $file) {
                ...
                $existingAttachments = array_filter($existingAttachments, function ($a) use ($file) {
                    return $a !== $file;
                });
            }
        }
        $lead = $this->tblleadModel::find($leadhistory->leadid);
        ...
        if ($request->hasFile('attachment')) {
            ...
            foreach ($request->file('attachment') as $attachment) {
                ...
                if ($attachment->move($dirPath, $attachmentname)) {
                    $newAttachments[] = $this->companyId . '/lead/callhistory/' . $timestamp . '/' . $attachmentname;
                }
            }
        ...
        $leadhistory->attachment = json_encode(array_values($existingAttachments)); // reset array keys
        $leadhistory->save();

        // ✅ Log activity
        $this->managerecentactivity($leadhistory->leadid, 'call_updated', 'Call history updated');

        return $this->successresponse(200, 'message', 'leadhistory succesfully Updated', 'leadid', $lead->id);
    }
    ...
    public function destroy(Request $request)
    {
        ...
        $history = $this->tblleadhistoryModel::find($request->history_id);

        if (!$history) {
            return $this->errorresponse(404, 'Lead history not found');
        }
        ...
        $history->save();
         ...
         // ✅ Log activity
        $this->managerecentactivity($leadid, 'call_deleted', 'Call history deleted');

        return $this->successresponse(200, 'message', 'Lead history deleted successfully','leadid',$leadid);
    }
