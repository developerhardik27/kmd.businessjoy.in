                                ...
                                <div class="col-12">
                                    <div class="custom-control custom-switch custom-control-inline">
                                        <input type="checkbox" name="track_quantity" class="custom-control-input"
                                            checked="" id="track_quantity" value="yes">
                ...
                init: function() {
                    ...
                    this.on("sending", function(file, xhr, formData) {
                        ...
                        if (uploadedimgs.indexOf(file.name) !== -1) {
                            // If the file has already been uploaded, cancel the upload
                            ...
                            // If the file hasn't been uploaded, proceed with adding custom data
                            formData.append("user_id", "{{ $user_id }}"); // Custom data
                            formData.append("company_id", "{{ $company_id }}"); // Custom data
                            formData.append("token",
                                "{{ session()->get('api_token') }}"); // Custom data (CSRF token)
                        }
                    });
                    ...
                    this.on("addedfile", function(file) {
                        ...
                        file.previewElement.addEventListener("click", function() {
                            if (file.fileUrl) {
                                const newTab = window.open(file.fileUrl, '_blank');
                                newTab.focus();
                            } else {
                                Toast.fire({
                                    icon: "error",
                    ...
                    this.on("removedfile", function(file) {
                        ...
                        // If the filename exists, remove it from the array
                        if (index > -1) {
