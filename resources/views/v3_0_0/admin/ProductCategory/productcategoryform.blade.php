...
@section('page_title')
    {{ config('app.name') }} - Add New Product Category
@endsection
@section('title')
...
@section('form-content')
    <form id="productcategoryform" name="productcategoryform">
        @csrf
            ...
            <div class="form-row">
                ...
                <div class="col-sm-6">
                    ...
                    <div class="tree-container" id="category-dropdown">
                        ...
                        <div class="tree-item">
                            <span data-id="main">Consider As A Main Category</span>
                        </div>
                    </div> 
    ...
    @isset($message)
        <script>
            $('document').ready(function() {
                // if  company has not any product category so user will be redirect here when he click on create product link
                alert('You have not any product category. Please first add product category!');
            });
        </script>
    @endisset
    <script>
        $('document').ready(function() {
                    ...
                    if (response.status == 200 && response.productcategory != '') {
                        $.each(response.productcategory, function(key, value) {
                            if (value.parent_id == null) {
                                ...
                                if (!$(`.parent_${value.parent_id}`).children('.arrow').length) {
                                    ...
                                    $(`.parent_${value.parent_id}`).children('.collapsible').append(`
                                        <div class="tree-item parent_${value.id}">
                                            <span data-id="${value.id}">${value.cat_name}</span> 
                                        </div> 
                                    `);
            ...
            // redirect on productcategory list page onclick cancel btn
            ...
            // submit product category form data
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.status == 200) {
                            ...
                            window.location =
                                "{{ route('admin.productcategory') }}"; // after succesfully data submit redirect on list page
