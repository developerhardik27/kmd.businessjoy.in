...
@section('page_title')
    {{ config('app.name') }} - Update Transporter Bill
@endsection
@section('title')
    Update Transporter Bill
@endsection
...
@section('form-content')
    <form id="transporterbillingupdateform" name="transporterbillingupdateform">
        @csrf
...
@push('ajax')
    <script>
        $('document').ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            ...
                            $('#party').val(transporterbill.party).select2();
                            $('#lr_number').val(transporterbill.lr_no);
                            $('#container_number').val(transporterbill.con_no);
                            $('#vehicle_number').val(transporterbill.vehicle_no);
                            $('#amount').val(transporterbill.amount);
                        } else if (response.status == 500) {
                            Toast.fire({
                                icon: "error",
                                title: response.message
            ...
            $('#transporterbillingupdateform').submit(function(event) {
                ...
                $.ajax({
                    type: 'PUT',
                    url: "{{ route('transporterbill.update', $edit_id) }}",
                    data: formdata,
                    success: function(response) {
