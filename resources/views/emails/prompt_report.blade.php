...
<head>
    ...
    <style type="text/css">
        ...
        * { box-sizing: border-box; }
        ...
        .badge-due { background: #fce4ec; color: #ad1457; }
...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background: linear-gradient(135deg, #97271f 0%, #253566 100%); padding: 40px 20px;">
    ...
    <div class="wrapper">
        ...
        <div class="title-bar">
            <h1>Prompt Report</h1>
            <p>Payment reminder for your records</p>
        </div>
        ...
        <div class="body-pad">
            <div class="details-card">
                ...
                <table>
                    <tr>
                        <td class="key">Buyer Name</td>
                        <td class="val">{{ $buyerName ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <td class="key">Email</td>
                        <td class="val">{{ $buyerEmail ?? 'N/A' }}</td>
                    </tr>
                    <tr>
                        <td class="key">Total Invoices</td>
                        <td class="val">{{ count($invoices ?? []) }}</td>
                    </tr>
                    @if(isset($companyName) && $companyName)
                    <tr>
                        <td class="key">Company</td>
                        <td class="val">{{ $companyName ?? 'N/A' }}</td>
                    </tr>
                    @endif
                </table>
            </div>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="right">{{ $invoice['currency_symbol'] ?? '' }}{{ number_format($invoice['amount'] ?? 0, 2) }}</td>
                            <td class="right">{{ $invoice['currency_symbol'] ?? '' }}{{ number_format($invoice['paid_amount'] ?? 0, 2) }}</td>
                            <td class="right">{{ $invoice['currency_symbol'] ?? '' }}{{ number_format($invoice['pending_amount'] ?? 0, 2) }}</td>
                            <td class="right">{{ $invoice['credit_days'] ?? '-' }}</td>
                            <td>{{ $invoice['expected_payment_date'] ?? '-' }}</td>
                            <td class="center">
                                @php $status = strtolower($invoice['status'] ?? ''); @endphp
                                @if($status === 'paid')
                                    <span class="badge badge-paid">Paid</span>
                                @elseif($status === 'part_payment')
                                    <span class="badge badge-part">Part Payment</span>
                                @elseif($status === 'cancel')
                                    <span class="badge badge-cancel">Cancel</span>
                                @elseif($status === 'due')
                                    <span class="badge badge-due">Over Due</span>
                                @else
        ...
        <div class="footer-msg">
            <p>{{ $customMessage ?? 'Please find the payment reminder details above.' }}<br>
