...
@section('advancefilter')
    <div class="col-sm-12 text-right px-4">
        ...
        @if (session('user_permissions.leadmodule.lead.add') == '1')
            <a href="{{route('admin.importfromexcel')}}" class="text-white btn btn-sm btn-primary m-0 ml-1 float-right" data-toggle="tooltip" data-placement="bottom"
                data-original-title="Import Data From Excel">
...
@push('ajax')
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '.lead-checkbox, #select-all-leads', toggleDeleteButton);
            ...
            function loaddata() {
                ...
                table = $('#data').DataTable({
                    ...
                    drawCallback: function(settings) {
                        ...
                        toggleDeleteButton(); // 👈 Recheck if any checkbox is selected
                        $('#select-all-leads').prop('checked', false); // 👈 Uncheck "select all" on page change
                    ...
                    success: function(response) {
                        if (response.status == 200 & response.leadhistory != '') {
                            $.each(response.leadhistory, function(key, value) {
                                ...
                                if (value.attachment) {
                                    let rawAttachment = Array.isArray(value.attachment) ?
                                        value.attachment :
