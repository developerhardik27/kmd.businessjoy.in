                        ...
                        },
                        {
                            ...
                            render: function(data, type, row) {
                                ...
                                @if (session('user_permissions.invoicemodule.customer.edit') == '1' ||
                                    ...
                                    let editCustomerUrl = "{{ route('admin.edit' . session('menu') . 'customer', '__customerid__') }}".replace('__customerid__', row.id);
                                    actionBtns += `
            ...
            $(document).on("click", ".del-btn", function() {
                ...
                showConfirmationDialog(
                    ...
                    () => {
                        // Success callback
                        loadershow();
                        let customerDeleteUrl = "{{ route('customer.delete', '__deleteId__') }}"
                            .replace(
                ...
                $.each(global_response.data, function(key, customer) {
                    if (customer.id == data) {
                        $('#details').append(`
                            ...
                            <tr>
                                <th>Address</th>       
                                <td>${customer.house_no_building_name || '-'}  ${customer.road_name_area_colony || ''}</td>
                            </tr>
                            <tr>
                                <th>Pincode</th>       
