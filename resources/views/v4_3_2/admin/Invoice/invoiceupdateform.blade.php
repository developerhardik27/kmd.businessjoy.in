...
@section('page_title')
    {{ config('app.name') }} - Update Invoice
@endsection
@section('title')
    Update Invoice
@endsection
...
@section('form-content')
    ...
    @if ($is_editable != 1)
        <p> This invoice is not editable </p>
    @else
        <form id="invoiceupdateform" name="invoiceupdateform">
            @csrf
            <div class="form-group">
                ...
                <div class="col-sm-4 mb-3">
                    <label for="companymaster_id">Companymaster</label><span style="color:red;">*</span>
                ...
                <div class="col-sm-4 mb-3">
                    ...
                    <span class="error-msg" id="error-transport_id" style="color: red"></span>
                </div>
                <input type="hidden" name="currency" id="currency" class="form-control" value="101" />
                ...
                <table id="data" class="table table-bordered table-striped text-center producttable">
                            ...
                            <div style="display:flex;justify-content: center;">
                                <span class="add_div mb-3 mr-2">
                                    <button type="button" data-toggle="tooltip" data-placement="bottom"
                                        data-original-title="Add New Row" class="btn btn-sm iq-bg-success">
                                        <i class="ri-add-fill">
                                ...
                                <span id="productdiv">
                                    <select class="form-control select2" id="product" name="product" class="mb-3 mr-2">

                                    </select>
                                </span>
                                <span id="lrdiv">
                                    <select class="form-control select2" id="lr" name="lr" class="mb-3 mr-2">
                        ...
                        <td>
                            <div class="d-flex justify-content-between">
                                <b><span class="currentcurrencysymbol"></span></b> <input class="disableinput" type="number"
                                    step="any" name="sgst" id="sgst" readonly required>
                            </div>
                        </td>
                    </tr>
                    <tr id="cgstline" class="text-right">
                        ...
                        <td>
                            <div class="d-flex justify-content-between">
                                <b><span class="currentcurrencysymbol"></span></b> <input class="disableinput" type="number"
                                    step="any" name="cgst" id="cgst" readonly required>
                            </div>
                        </td>
                    </tr>
                    <tr id="gstline" class="text-right">
                        ...
                        <td>
                            <div class="d-flex justify-content-between">
                                <b><span class="currentcurrencysymbol"></span></b> <input class="disableinput" type="number"
                                    step="any" name="gst" id="gst" readonly required>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr id="grandtotalline" class="text-left">
                        ...
                        <td>
                            <div class="d-flex justify-content-between">
                                ...
                                <input class="disableinput" type="number" step="any" name="grandtotal" id="grandtotal" readonly
                                    required>
                            </div>
                ...
                <div class="form-row">
                    <div class="col-sm-12">
                        <button id="cancelbtn" type="button" data-toggle="tooltip" data-placement="bottom"
                            data-original-title="Cancel" class="btn btn-secondary btn-rounded float-right">Cancel</button>
                        ...
                        <button type="submit" data-toggle="tooltip" data-placement="bottom" data-original-title="Update Details"
    ...
    @else
        ...
        <script>
            $('document').ready(function () {
                ...
                // get consigneecopy data 
                ajaxRequest('GET', "{{ route('consignorcopy.index') }}", {
                    token: API_TOKEN,
                    company_id: COMPANY_ID,
                    user_id: USER_ID
                }).done(function (response) {
                    ...
                    if (response.status == 200 && response.data != '') {
                        ...
                        $('#lr').select2({
                            ...
                            search: true,
                        });  // search bar in lr list 
                        if (consignorcopycount < 1) {
                            $('#lrdiv').hide();
                        }
                    } else {
                        // Toast.fire({
                        ...
                        //     title: response.message
                        // });  
                        // $('#lr').append(`<option disabled selected>No Product found</option>`);
                        $('#lrdiv').hide();
                    }
                ...
                function transports(transportid = 0) {
                    ...
                    $('#transport_id').html(`
                        <option selected="" value=0 disabled=""> Select transport</option>
                    `);
                    ajaxRequest('GET', "{{ route('transport.index') }}", {
                        token: API_TOKEN,
                        company_id: COMPANY_ID,
                        user_id: USER_ID,
                    }).done(function(response) {
                        if (response.status == 200 && response.data != '') {
                            // You can update your HTML with the data here if needed
                            $.each(response.data, function(key, value) {
                                const transportDetails = [value.name, value.mobile_1, value.email]
                                    .filter(Boolean).join(' - ');
                                $('#transport_id').append(`
                                        <option  data-gstno='${value.gst_no}' value='${value.id}'>${transportDetails}</option>
                                    `);
                            });
                            $('#transport_id').val(transportid);
                            $('#transport_id').prop('disabled', true);
                            if (transportid != 0) { // get transport data if its new added from add new transportform
                                loadershow();
                                var selectedOption = $('#transport').find('option:selected');
                                let transportSearchUrl = "{{ route('party.search', '__transportId__') }}"
                                    .replace('__transportId__', transportid);
                                ajaxRequest('GET', transportSearchUrl, {
                                    token: API_TOKEN,
                                    company_id: COMPANY_ID,
                                    user_id: USER_ID
                                }).done(function(response) {
                            ...
                            $('#transport').append(`<option disabled '>No Data found </option>`);
                        }
                        loaderhide();
                    }).fail(function(xhr) {
                        loaderhide();
                ...
                function customers(customerid = 0) {
                    ...
                    $('#customer').html(`
                        <option selected="" value=0 disabled=""> Select Buyer</option>
                    `);
                    ajaxRequest('GET', "{{ route('buyer.index') }}", {
                        token: API_TOKEN,
                        company_id: COMPANY_ID,
                        user_id: USER_ID,
                    }).done(function(response) {
                        if (response.status == 200 && response.party != '') {
                            ...
                            if (customerid !=
                                ...
                                let customerSearchUrl = "{{ route('party.search', '__customerId__') }}"
                ...
                function companymaster(companymaster_id = 0) {
                    
                    $('#companymaster_id').html(`
                        <option selected="" value=0 disabled=""> Select Companymaster</option>
                    `);
                    ajaxRequest('GET', "{{ route('companymaster.index') }}", {
                        ...
                        if (response.status == 200 && response.data != '') {
                            ...
                            if (companymaster_id !=
                                ...
                                let companymaster_idSearchUrl = "{{ route('companymaster.search', '__companymaster_idId__') }}"
                                    .replace('__companymaster_idId__', companymaster_id);
                                ajaxRequest('GET', companymaster_idSearchUrl, {
                                    token: API_TOKEN,
                                    company_id: COMPANY_ID,
                                    user_id: USER_ID
                                }).done(function(response) {
                ...
                ajaxRequest('GET', invoiceEditUrl, {
                    token: API_TOKEN,
                    company_id: COMPANY_ID,
                    user_id: USER_ID
                }).done(function (response) {
                    if (response.status == 200) {
                        ...
                        $.each(productdetails, function (key, value) {
                            ...
                            targetRow.append(`
                            <tr class="iteam_row_${dynamicidcount}" data-inventory="${value.inventory_product_id || null}">
               ...
               $('#customer').on('change', function() {
                        ...
                        customerSearchUrl = "{{ route('party.search', '__customerId__') }}".replace(
                ...
                $('#transport_id').on('change', function() {
                        ...
                        if (transport_id == 'add_transport_id') {
                            $('#exampleModalScrollable').modal('show');
                        }

                        transport_idSearchUrl = "{{ route('party.search', '__transport_id__') }}".replace(
                            '__transport_id__', transport_id);

                        ajaxRequest('GET', transport_idSearchUrl, {
                            token: API_TOKEN,
                            company_id: COMPANY_ID,
                            user_id: USER_ID
                        }).done(function(response) {
                ...
                $('#companymaster_id').on('change', function() {
                        ...
                        companymaster_idSearchUrl = "{{ route('companymaster.search', '__companymaster_id__') }}".replace(
                            '__companymaster_id__', companymaster_id);
                    ...
                    <tr class="iteam_row_${addname}" data-inventory="null">
                        ...
                        columnData => {
                            ...
                            if (columnData.column_type === 'time') {
                                return `
                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                        <input type="time" name="${columnName}_${addname}" value="${columnData.default_value || ''}" id='${columnName}_${addname}' class="form-control iteam_${columnName} ">
                                    </td>
                                `;
                            } else if (columnData.column_type === 'number' || columnData.column_type === 'percentage' || columnData.column_type === 'decimal') {
                                return `
                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                        <input type="number" name="${columnName}_${addname}" value="${columnData.default_value || ''}" id='${columnName}_${addname}' data-id = ${addname} class="form-control iteam_${columnName} counttotal calculation" step="any" min=0>
                                    </td>
                                `;
                            } else if (columnData.column_type === 'longtext') {
                                return `
                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                        <textarea name="${columnName}_${addname}" id='${columnName}_${addname}' class="form-control iteam_${columnName} " rows="1">${columnData.default_value || ''}</textarea>
                                    </td>
                                `;
                            } else {
                                return `
                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                        <input type="text" name="${columnName}_${addname}" value="${columnData.default_value || ''}" id='${columnName}_${addname}' class="form-control iteam_${columnName} " placeholder="${columnData.column_name}">
                                    </td>`;
                            }
                        }).join('')
                        }
                ...
                function duplicatediv(id, inventoryproduct) {
                    ...
                    <tr class="iteam_row_${addname}" data-inventory="${inventoryproduct}">
                        ...
                        if (columnData.column_type === 'time') {
                            ...
                            return `
                                <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                    <input type="number" name="${columnName}_${addname}" value="${$('#' + columnName + '_' + id).val() || `${columnData.default_value || ''}`}"  id='${columnName}_${addname}' data-id = ${addname} class="form-control iteam_${columnName} counttotal calculation" step="any"  min=0>
                                </td>
                            `;
                        } else if (columnData.column_type === 'longtext') {
                            return `
                            ...
                            <td>
                                <input type="number" name="Amount_${addname}" value="${$('#Amount' + '_' + id).val() || ''}" id="Amount_${addname}" class="form-control iteam_Amount changeprice calculation" placeholder="Amount" data-id = ${addname} step="any" min=0 required>
                            </td>   
                            <td>
                                <span class="table-up">
                                    <a href="#!" class="indigo-text">
                                    ...
                                    $('#add_new_div').append(
                                        `<tr class="iteam_row_${addname}" data-inventory="${value.id}">
                                            ...
                                            if (columnData.column_type === 'time') {
                                                inputcontent = `
                                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                                        <input type="time" name="${columnName}_${addname}" value="${productColumnValue || `${columnData.default_value || ''}`}" id='${columnName}_${addname}' class="form-control iteam_${columnName}">
                                                    </td>
                                                `;
                                            } else if (columnData.column_type === 'number' || columnData.column_type === 'percentage' || columnData.column_type === 'decimal') {
                                                inputcontent = `
                                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                                        <input type="number" name="${columnName}_${addname}" value="${productColumnValue || `${columnData.default_value || ''}`}" ${validation} id='${columnName}_${addname}' data-id=${addname} class="form-control iteam_${columnName} counttotal calculation" step="any" min=0>
                                                        ${text || ''}
                                                    </td>
                                                `;
                                            } else if (columnData.column_type === 'longtext') {
                                                ...
                                                inputcontent = `
                                                    <td class="invoicesubmit ${(columnData.is_hide === 1) ? 'd-none' : ''} ">
                                                        <input type="text" name="${columnName}_${addname}" value="${productColumnValue || `${columnData.default_value || ''}`}" id='${columnName}_${addname}' class="form-control iteam_${columnName}" placeholder="${columnData.column_name}">
                                                    </td>
                                                `;
                                            }
                                            return inputcontent;
                ...
                $('#lr').on('change', function () {
                    let selectedlr = $(this).val();
                    addname++;

                    if (lrcolumnlinks.length > 0) {
                        if (lrdata != null) {
                            $.each(lrdata, function (key, value) {

                                if (value.id == selectedlr) {
                                    $('#add_new_div').append(
                                        `<tr class="iteam_row_${addname}" >
                                                        ${allColumnData.map(columnData => {

                                            let columnName = columnData.column_name.replace(/\s+/g, '_');
                                            let inputcontent = '';
                                            let lrColumnValue = '';

                                            // match LR column mapping
                                            let matchingLink = lrcolumnlinks.find(
                                                link => link.invoice_column === columnData.column_name
                                            );

                                            if (matchingLink) {
                                                lrColumnValue = value[matchingLink.lr_column] ?? '';
                                            }

                                            if (columnData.column_type === 'number' ||
                                                columnData.column_type === 'decimal' ||
                                                columnData.column_type === 'percentage') {

                                                inputcontent = `
                                                        <td class="invoicesubmit ${columnData.is_hide === 1 ? 'd-none' : ''}">
                                                            <input type="number"
                                                                step="any"   min="0" data-id="${addname}"value="${lrColumnValue !== undefined && lrColumnValue !== null
                                                        ? lrColumnValue
                                                        : (columnData.default_value ?? '')}"
                                                                name="${columnName}_${addname}"
                                                                id="${columnName}_${addname}"
                                                                class="form-control iteam_${columnName} counttotal calculation">
                                                        </td>`;
                                            }
                                            else {
                                                inputcontent = `
                                                        <td class="invoicesubmit ${columnData.is_hide === 1 ? 'd-none' : ''}">
                                                            <input type="text"
                                                                value="${lrColumnValue || columnData.default_value || ''}"
                                                                name="${columnName}_${addname}"
                                                                id="${columnName}_${addname}"
                                                                class="form-control iteam_${columnName}"
                                                                placeholder="${columnData.column_name}">
                                                        </td>`;
                                            }
                                            return inputcontent;
                                        }).join('')}
                        ...
                        Toast.fire({
                            icon: "info",
                            title: "LR column mapping required"
                        });
                    }
                ...
                $('#invoiceupdateform').submit(function (event) {
                    ...
                    $('tbody#add_new_div tr').each(function (index, row) {
                        ...
                        const inventoryproduct = $(row).data('inventory'); // it would be product id or null
                        ...
                        $.each(allColumnNames, function (key, columnName) {
                            ...
                            if (oldinputId) {
                                ...
                                if (!oldproductrowData[oldinputId]['inventoryproduct']) {
                                    oldproductrowData[oldinputId]['inventoryproduct'] = inventoryproduct;
                                }

                            } else {
                                rowData[columnNameWithUnderscores] = inputValue;
                                if (!rowData['inventoryproduct']) {
                                    rowData['inventoryproduct'] = inventoryproduct;
                                }
                            }
                        });
                    ...
                    var invoice_number = $('#inv_number').val();
                    ...
                    let invoiceUpdateUrl = "{{route('invoice.update', '__editId__')}}".replace('__editId__', edit_id);
                    $.ajax({
                        type: 'PUT',
